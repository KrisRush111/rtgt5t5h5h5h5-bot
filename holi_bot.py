...
SERVER_URL = "https://server-for-holi.onrender.com/update_user"
...
def send_user_data(user_name):
    try:
        response = requests.post(SERVER_URL, json={"user_name": user_name})
...
async def cmd_start(message: types.Message):
    ...
    send_user_data(user_name)

    builder = InlineKeyboardBuilder()
    builder.button(text="Подписаться на канал", url="https://t.me/holiarus")
    builder.button(text="Играть в 1 клик🐵", url=MINI_APP_URL)
    builder.adjust(1)
